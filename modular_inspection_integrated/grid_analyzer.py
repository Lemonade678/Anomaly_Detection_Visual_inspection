"""Grid-based image analyzer for 9-part defect detection (from V1).
...
from .config import GridAnalyzerConfig


class GridAnalyzer:
    """Analyzes images by dividing them into a 3x3 grid."""

    def __init__(self, config: GridAnalyzerConfig = None):
        """Initialize the grid analyzer.
        
        Args:
            config: Grid analyzer configuration
        """
        if config is None:
            config = GridAnalyzerConfig()
        
        self.grid_size = config.grid_size
        self.pixel_diff_threshold = config.pixel_diff_threshold
        self.count_threshold = config.count_threshold
        self.ssim_threshold = config.ssim_threshold
        self.min_anomaly_segments = config.min_anomaly_segments
    ...
    def divide_image(self, image: np.ndarray) -> List[np.ndarray]:
        ...
        Returns:
            List of image segments in order (left-to-right, top-to-bottom)
        """
    ...
    def analyze_segment_pair(self, golden_segment: np.ndarray, test_segment: np.ndarray) -> Dict:
        ...
        try:
            aligned_segment, (dx, dy), confidence, valid_mask = align_images(
                golden_segment, test_segment
            )
            ...
            result['confidence'] = pixel_result.get('confidence', pixel_result['area_score'] / 100.0)
    ...
    def analyze_images(self, golden_image: np.ndarray, test_image: np.ndarray) -> Dict:
        """Perform full grid analysis on image pair.
        ...
        Returns:
            Dictionary with results for all segments and overall verdict
        """
        ...
        num_segments = self.grid_size * self.grid_size

        for idx in range(num_segments):
            segment_result = self.analyze_segment_pair(
                golden_segments[idx], test_segments[idx]
            )
        ...
        overall_defect_score = (anomaly_count / num_segments) * 100
        average_confidence = total_confidence / num_segments

        final_verdict = 'Anomaly' if anomaly_count >= self.min_anomaly_segments else 'Normal'
    ...
    def visualize_results(self, image: np.ndarray, analysis_results: Dict) -> np.ndarray:
        """Create visualization of analysis results on the image.
