"""Image I/O helpers.
Contains `read_image(path)` which tries rawpy, OpenCV and PIL fallbacks and returns a BGR np.ndarray.
"""
import os
import cv2
import numpy as np
...
def read_image(path: str) -> np.ndarray:
    ...
    if ext in RAW_EXTS:
        try:
            ...
            raise RuntimeError("Detected RAW file but 'rawpy' is not installed. Install with: pip install rawpy") from e
        except Exception:
